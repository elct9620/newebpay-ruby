...
RSpec.describe OffsitePayments::Integrations::Newebpay::Notification do
  let(:query_string) do
    'Status=SUCCESS&' \
  ...
  let(:notification) { described_class.new(query_string) }
  ...
  describe '#gross_cents' do
    subject { notification.gross_cents }

    it { is_expected.to eq(4000.0) }
